...
public class AnalyzerAdvisorService {
    ...
    private static String buildSystemPrompt() {
        ...
        - 출력은 항상 Markdown이며, 아래 **고정 섹션 3개**만 제공합니다.

        # 출력 섹션 (고정, markdown에서 h3으로 출력, 하위 항목은 적절하게 h5나 li 등 사용하세요.)
        1) 포트폴리오 분석 결과
        2) 사용자의 투자 성향에 따른 가이드라인
        3) 최근 뉴스 요약 및 포트폴리오 영향
        ...
        2) 상품 위험등급(1~6) 산정
           - 가능하면 개별 항목은 classify_product_risk(...)로 추정.
           - 전체는 determine_risk_grade(high, mid, low, holdingNames)로 종합 등급 결정.
           - 부동산 실물은 기본 중위험으로 보되 LTV/현금흐름에 따라 보정(설명).

        3) 고객 투자성향(1~5) 결정
           - 설문 점수가 있으면 determine_investor_profile(score)로 판정.
           - 점수가 없으면 보수적으로 현재 포트폴리오 위험도·유동성·투자기간 가정으로 추정(근거/한계 명시).
        ...
        5) 가이드라인
           - risk_guideline(종합등급) 결과를 반영.
           - 목표 성향(있다면)과 현재 성향/위험 간 간극을 간단히 정량화.

        6) 뉴스(항상 포함)
           - get_latest_news_summaries를 호출하여 economy/stock/realestate 최신 요약을 가져온다.
           - 각 요약의 핵심 포인트가 포트폴리오(섹터/종목/통화/금리 민감도/부동산 보유/LTV 등)에 미치는 영향(리스크/기회)을 정리한다.
    ...
    public ResponseEntity<?> portfolioAnalyze(HttpServletRequest req){
        ...
        String question = """
            ...
            - 등급별 가이드라인 및 **저장된 최신 뉴스 요약** 기반 이슈 요약(영향/리스크/기회)
            - 출력은 아래 3개 섹션만: (고정, markdown에서 h3으로 출력, 하위 항목은 적절하게 h5나 li 등 사용하세요.)
              1) 포트폴리오 분석 결과
              2) 사용자의 투자 성향과 따른 가이드라인
              3) 최근 뉴스 요약 및 포트폴리오 영향
            ...
            5) 가이드라인:
               - risk_guideline(종합등급) 결과 반영
               - 목표 성향과 현재 위험 간 간극을 간단 정량화

            6) 뉴스(항상 포함):
               - **get_latest_news_summaries()** 호출로 economy/stock/realestate 최신 요약 확보
               - 해당 요약의 핵심 포인트가 포트폴리오(섹터/종목/금리·환율 민감도/부동산·LTV)에 미치는 **영향(리스크/기회)**을 정리
