...
/**
 * <h3>StateAssetService</h3>
 */
@Slf4j
@RequiredArgsConstructor
@Service
public class StatsAssetService {
    private final StatsAssetDataRepository statsAssetDataRepository;
    private final StatsIncomeDataRepository statsIncomeDataRepository;
    private final StatsConsumptionDataRepository statsConsumptionDataRepository;
    private final UserRepository userRepository;
    private final JwtService jwtService;

    /**
     * <h3>searchAssetData</h3> 1. 자산 통계 조회
     * @param statsDataRequest : age, c2, itemId, year
     * @param httpServletRequest
     * @return ResponseEntity.ok (성공 시)
     */
    public ResponseEntity<?> searchAssetData(StatsDataRequest statsDataRequest, HttpServletRequest httpServletRequest) {
        String userUid = jwtService.getUserUid(httpServletRequest);
        Integer age = statsDataRequest.getAge();
        Integer year = statsDataRequest.getYear();
        ...
        List<StatsAssetData> entityList =
                statsAssetDataRepository.findByC1AndC2NameInAndItemIdInAndYear(c1, c2Names, itemIds, year);
        ...
        List<StatsDataResponse> responseList = entityList.stream()
                .map(e -> {
     ...
     */
    public ResponseEntity<?> searchIncomeData(StatsDataRequest statsDataRequest, HttpServletRequest httpServletRequest) {
        String userUid = jwtService.getUserUid(httpServletRequest);
        Integer age = statsDataRequest.getAge();
        Integer year = statsDataRequest.getYear();
        ...
        List<StatsIncomeData> entityList =
                statsIncomeDataRepository.findByC1AndC2NameInAndItemIdInAndYear(c1, c2Names, itemIds, year);
     ...
     * @return ResponseEntity.ok (성공 시)
     */
    public ResponseEntity<?> searchConsumptionData(StatsDataRequest statsDataRequest, HttpServletRequest httpServletRequest) {
        String userUid = jwtService.getUserUid(httpServletRequest);
        Integer age = statsDataRequest.getAge();
        Integer year = statsDataRequest.getYear();
        ...
        List<StatsConsumptionData> entityList =
                statsConsumptionDataRepository.findByC1AndC2NameInAndItemIdInAndYear(c1, c2Names, itemIds, year);
